        ...
        InfoType::FileSystem => {
            ...
            match req.file_information_class {
                ...
                ic::FS_SIZE_INFORMATION => {
                    let file_system_info = match backend.file_system_info().await {
                        Ok(Some(info)) => info,
                        Ok(None) => return HandlerResponse::err(ntstatus::STATUS_NOT_SUPPORTED),
                        Err(error) => return HandlerResponse::err(error.to_nt_status()),
                    };
                    ic::encode_fs_size_information(
                        file_system_info.total_allocation_units,
                        file_system_info.caller_available_allocation_units,
                        file_system_info.sectors_per_allocation_unit,
                        file_system_info.bytes_per_sector,
                    )
                }
                ...
                ic::FS_FULL_SIZE_INFORMATION => {
                    ...
                    ic::encode_fs_full_size_information(
                        file_system_info.total_allocation_units,
                        file_system_info.caller_available_allocation_units,
                        file_system_info.actual_available_allocation_units,
                        file_system_info.sectors_per_allocation_unit,
                        file_system_info.bytes_per_sector,
                    )
                }
