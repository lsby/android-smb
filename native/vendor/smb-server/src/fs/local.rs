...
use crate::backend::{
    BackendCapabilities, DirEntry as SmbDirEntry, FileInfo, FileSystemInfo, FileTimes, Handle,
    OpenIntent, OpenOptions, ShareBackend,
};
...
fn query_file_system_info(root: &Dir) -> io::Result<FileSystemInfo> {
    let mut stats = MaybeUninit::<libc::statvfs>::uninit();
    // SAFETY: `stats` points to writable storage for one `statvfs` value and
    // `root` owns a valid directory file descriptor for the duration of the
    // call. A successful return initializes every field before `assume_init`.
    if unsafe { libc::fstatvfs(root.as_raw_fd(), stats.as_mut_ptr()) } != 0 {
        return Err(io::Error::last_os_error());
    }
    // SAFETY: `fstatvfs` returned success above.
    let stats = unsafe { stats.assume_init() };
    let allocation_unit_size = if stats.f_frsize == 0 {
        stats.f_bsize
    } else {
        stats.f_frsize
    };
    let (sectors_per_allocation_unit, bytes_per_sector) =
        if allocation_unit_size >= 512 && allocation_unit_size % 512 == 0 {
            (u32::try_from(allocation_unit_size / 512).unwrap_or(1), 512)
        } else {
            (1, u32::try_from(allocation_unit_size).unwrap_or(4096))
        };
    Ok(FileSystemInfo {
        total_allocation_units: stats.f_blocks,
        caller_available_allocation_units: stats.f_bavail,
        actual_available_allocation_units: stats.f_bfree,
...
impl ShareBackend for LocalFsBackend {
    ...
    async fn file_system_info(&self) -> SmbResult<Option<FileSystemInfo>> {
        let root = Arc::clone(&self.root);
        let info = spawn_blocking(move || query_file_system_info(&root))
            .await
            .map_err(join_to_io)
            .map_err(io_to_smb)?
            .map_err(io_to_smb)?;
